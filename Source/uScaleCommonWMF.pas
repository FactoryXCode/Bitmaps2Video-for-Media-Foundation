...
type
  ...
  TContributor = record
    Min: Integer; // Min: start source pixel
    High: Integer; // High+1: number of source pixels to contribute to the result.
  ...
  TResamplingThreadSetup = record
    ...
    ContribsY: TContribArray; // contributors horizontal/vertical
    rStart: PByte;
    rTStart: PByte; // Scanline for row 0
    xmin: Integer;
    xmax: Integer;
    ThreadCount: Integer;
    xminSource: Integer;
    xmaxSource: Integer;
    ymin: TIntArray;
    ymax: TIntArray;  //ymin, ymax for each thread
...
/// <summary> Initializes the default resampling thread pool.
/// If already initialized, it does nothing.
/// If not called, the default pool is initialized at
/// the first use of a parallel procedure, causing a delay. </summary>
procedure InitDefaultResamplingThreads();

/// <summary> Frees the default resampling threads.
/// If they are initialized and not finalized the Finalization of uScale(FMX) will do it. </summary>
...
begin
  ...
  for x := 0 to TargetSize - 1 do
    begin
      ...
      Contribs[x].Min := Min(Max(TrueMin,
                                 0),
                             SourceSize - 1);
        ...
        begin
          ...
          for j := 0 to High do
            begin
             ...
             dw := PointCountInv * (x2 - x1) * (FT(x1) + FT(x2) + PointCountMinus2 * FT(x3));
...
begin
  if (acm in [amIndependent, amIgnore]) then
    ...
    begin
      alpha := Weight * ps.a;
      Inc(Cache.b,
          ps.b * alpha div PreMultPrecision);
      Inc(Cache.g,
          ps.g * alpha div PreMultPrecision);
      Inc(Cache.r,
          ps.r * alpha div PreMultPrecision);
      Inc(Cache.a,
          alpha);
    end;
end;
