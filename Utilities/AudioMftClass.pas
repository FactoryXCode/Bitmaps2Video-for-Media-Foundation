...
type
  ...
  TMP3ProfileInfo = record
   ...
   TPCM_profiles = array of TPCMProfileInfo;
...
type
  ...
  TAudioMft = class(TObject)
    private
      ...
      /// <summary>
      /// Set pa_PCM_profiles length to zero.
      /// </summary>
      procedure ResetPCMProfileInfoArray();
      /// <summary>
    ...
    public
      ...
      // Audio Types Profile array's
      property  PCMprofiles: TPCM_profiles read pa_PCM_profiles;
  ...
  /// Error handler.
  /// </summary>
  /// <param name="hr">HResult code from caller.</param>
  /// <param name="customErrMsg">Enter a custom error message or '!' for the localised system error message (default).</param>
  procedure CheckHR(hr: HRESULT;
                    const customErrMsg: string = '');
...
begin
  if (hr <> S_OK) then
    begin
      if customErrMsg = '' then
        sMsg := SysErrorMessage(hr)
      else
        sMsg := customErrMsg;
      // Write to IDE
      ...
      // Exit the current method to simulate the 'goto done' effect.
      Exit;
    end;
end;
...
procedure TAudioMft.ResetPCMProfileInfoArray();
begin
  SetLength(pa_PCM_profiles,
            0);
end;
